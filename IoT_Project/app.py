...
def update_content(n_intervals, name_value, email_value, temp_value, threshold_value):
    global currCard
    global isNewCard
...
def create_user():
    updated_image = [html.Img(id='pfp-image', src="/assets/pfp/default.jpg"),]
    
    updated_content = [html.Div(children=[
                    html.Label('Name: ', className='info-labels'),
                    html.Div(children=[
                        dcc.Input(id='name-input', className='info-inputs', value="Default User"),
                    ]),
                ], className='info-all'),
                html.Div(children=[
                    html.Label('Email: ', className='info-labels'),
                    html.Div(children=[
                        dcc.Input(id='email-input', className='info-inputs', value="etotesto1234@outlook.com"),
                    ]),
                ], className='info-all'),
                html.Div(children=[
                    html.Label('Preferred Temperature (°C): ', className='info-labels'),
                    html.Div(children=[
                        dcc.Input(id='temp-input', className='info-inputs', value="22"),
                    ]),
                ], className='info-all'),
                html.Div(children=[
                    html.Label('Light Activation Threshold: ', className='info-labels'),
                    html.Div(children=[
                        dcc.Input(id='threshold-input', className='info-inputs', value="4"),
                    ]),
                ], className='info-all'),
            ]
...
def toggle_led(n):
    ...
    if row:
        ...
        if (int(lightVal) < 4 and emailSentLight == 2):
            GPIO.output(LED, GPIO.HIGH)
            light_intensity = lightVal
            led_image = '/assets/light_on.png'
            email_text = "The light has been turned on, and a notification email has been sent."
...
def send_light_email(n):
    ...
    if row:
        ...
        if (emailSentLight == 1):
            emailSentLight = 2
            send_email_light("etotesto1234@outlook.com")
...
def send_motor_email(n):
    ...
    if row:
        ...
        if (emailSentMotor == 1):
            emailSentMotor = 2
            send_email(email, temperature)
    else:
        email_identifier = generate_identifier()
        
        if (emailSentMotor == 1):
            emailSentMotor = 2
            send_email("etotesto1234@outlook.com", temperature)
...
def checkHumAndTemp(n):
    ...
    if row:
        ...
        if (temperature > 22 and emailSentMotor == 0):
            emailSentMotor = 1
...
def send_email(email, temperature):
    imap_server = "outlook.office365.com"
    with smtplib.SMTP(imap_server, 587) as smtp:
        ...
        body = f'The current temperature is over {temperature}. Would you like to turn on the fan?'
        msg = MIMEText(body)
        msg['Subject'] = subject
        msg['From'] = username
        msg['To'] = email
